...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  9 16:49:57 2021
...
mesh = trimesh.load("../examples/data/cube.stl")
...
for i in range(triangles.shape[0]):
    ...
    ax.plot3D(triangle_center[0], triangle_center[1], triangle_center[2],
              "blue", marker='x')
    norm_coords = np.vstack((triangle_center, triangle_center+180*normals[faces_inds[i]]))
    ax.plot3D(norm_coords[:, 0], norm_coords[:, 1], norm_coords[:, 2], "blue")

source = np.array([-5000.00, 0.00, 0.00])
pixel  = np.array([200.00, -144.50, 144.50])
ray_coords = np.vstack((source + (pixel-source)*0.90, pixel))
